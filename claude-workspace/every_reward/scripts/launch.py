"""Launch the Every Reward server. Writes a pidfile so Stop can find us."""
import argparse
import os
import sys
...
def main() -> int:
    ...
    try:
        ...
        print("Dependencies missing — run 'Install Every Reward.command' first.")
        return 1

    from backend import config
    cfg = config.load()
    port = args.port or cfg.get("port", 8850)
    ...
    print(f"Every Reward → {scheme}://{args.host}:{port}  (pid {os.getpid()})")
    try:
