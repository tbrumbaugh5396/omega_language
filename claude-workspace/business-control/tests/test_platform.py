...
ok(len(_caps_pb) == 31 and {c["price"] for c in _caps_pb} == {20, 30, 50},
   "the price book parses into code — 31 capabilities on three bands, one "
...
ok('class="band band-light"' in _shop
   and _shop.count("class=\"band band-") == 31,
...
ok(_pbld["core"] == 50 and len(_pbld["groups"]) == 5
   and sum(len(g["items"]) for g in _pbld["groups"]) == 31
   and [v["rate"] for v in _pbld["volume"]] == [0.0, 0.08, 0.15, 0.25],
   "the configurator is served the book: 31 capabilities in five groups, "
...
ok(len(_fbrd["cap_catalog"]) == 31 and _fbrd["core_price"] == 50,
