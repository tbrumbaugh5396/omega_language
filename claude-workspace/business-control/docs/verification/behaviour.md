# Behaviour walkthroughs — <release or date>

*What a person sees at the keyboard, on the built artifact, done by someone who did not build the change. Copy the walkthrough block once per flow; add steps freely. Every unexpected thing is a defect, even when the step passed.*

| | |
|---|---|
| **Release** | |
| **Commit** | |
| **Build under test** | (URL, port, seed used) |
| **Walked by** | (not the author) |
| **Browser · widths** | |
| **Date** | |

## Before you start

- [ ] Fresh browser profile or private window
- [ ] The build is the commit above (check the version or the commit shown in the app)
- [ ] Seeded as the walkthrough says, or empty on purpose
- [ ] Console open, so errors are seen

## Walkthroughs

<!-- repeat: one block per flow -->
### W<n> · <Flow name>

*Who is doing this, starting where, with what already true.*

| Step | Do | Expected | Result (desktop) | Result (narrow) |
|---|---|---|---|---|
| 1 | | | | |
| 2 | | | | |
| 3 | | | | |

Notes: (anything odd — timing, layout, wording, console)

<!-- end repeat -->

## Defects found

| # | Walkthrough · step | What happened | Filed as |
|---|---|---|---|
| | | | |

## Sign-off

| Release | Commit | Walked by | Date | Passed / total steps |
|---|---|---|---|---|
| | | | | |
