...
| `client_ip_header` | `cf-connecting-ip` **only behind a CDN** | a CDN answers for every caller, so `X-Forwarded-For` behind one says "Cloudflare" and every shop looks like the same network. This names the header carrying the caller the CDN actually saw. Read only on requests that already came through the local proxy, so it cannot be posted from outside. Leave unset without a CDN |
| `kiosk_networks` | `["203.0.113.0/24", "192.0.2.0/24"]` | the networks your shops call from. With them declared, a kiosk setup link works only from a listed network, and only from the same one it was minted on — so an owner can make a link at head office for a shop across the country and nobody at head office can spend it. Without them, the check falls back to comparing a `/24` either side of the minting address, which is a guess that stops working the moment a CDN is in front |
