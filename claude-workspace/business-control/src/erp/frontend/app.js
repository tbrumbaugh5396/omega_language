...
async function learningLibrary() {
  ...
  $("#lib-add").onclick = () => {
    modal(`<h3>Add a library item</h3>
      <label>Name</label><input id="li-name">
      <div class="row2">
        <div><label>Kind</label><select id="li-kind">
          <option value="book">book</option>
          <option value="material">material</option>
          <option value="equipment">equipment</option></select></div>
        <div><label>Copies</label><input id="li-copies" type="number" value="1"></div>
      </div>
      <label>Notes <span class="dim">(optional)</span></label>
      <input id="li-notes">
      <div class="modal-acts">
        <button class="btn alt" data-close>Cancel</button>
        <button class="btn" id="li-save">Add</button>
      </div>`);
    $("#li-save").onclick = async () => {
      try {
        await api("/api/learning/library/items", { body: {
          name: $("#li-name").value, kind: $("#li-kind").value,
          copies: +$("#li-copies").value || 1,
