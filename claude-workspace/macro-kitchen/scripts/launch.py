"""Launch the MacroKitchen server. Writes a pidfile so Stop can find us."""
import argparse
import os
import sys
...
def main() -> int:
    ...
    try:
        ...
        print("Dependencies missing — run 'Install MacroKitchen.command' first.")
        return 1

    from backend import config
    cfg = config.load()
    port = args.port or cfg.get("port", 8900)

    # Make sure icons exist (needed for PWA install).
    icons = ROOT / "src" / "frontend" / "icons" / "icon-192.png"
    if not icons.exists():
        import subprocess
    ...
    print(f"MacroKitchen → {scheme}://{args.host}:{port}  (pid {os.getpid()})")
    try:
        uvicorn.run("backend.main:app", host=args.host, port=port,
