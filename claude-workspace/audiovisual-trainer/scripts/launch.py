"""Launch the AV Trainer server. Writes a pidfile so Stop can find us."""
import argparse
import os
import sys
...
def main() -> int:
    ...
    try:
        ...
        print("Dependencies missing — run 'Install AV Trainer.command' first.")
        return 1

    from backend import config
    cfg = config.load()
    port = args.port or cfg.get("port", 8910)
    ...
    print(f"AV Trainer → {scheme}://{args.host}:{port}  (pid {os.getpid()})")
    try:
