...
![Value, first derivative, and second derivative of linear, cubic, and quintic interpolants](figures/smoothstep-continuity.svg)

Read the middle panel for why `smoothstep` beats linear — the derivative goes to zero at both ends, so there's no kink where the ramp meets the flat region. Read the right panel for why Perlin needed the quintic: the cubic's second derivative is nonzero at $t=0$ and $t=1$, which is exactly the lattice boundary in a noise function.
