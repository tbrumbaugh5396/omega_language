...
![sRGB decoding curve, and the Weber ratio per code step for linear vs sRGB 8-bit encoding](figures/srgb-transfer.svg)

The right panel is the argument in one picture. A linear 8-bit encoding's relative step size explodes toward black — the bottom codes are separated by ratios of 50% and more, far above any visibility threshold. sRGB encoding flattens the curve to roughly 1% across most of the range, which is close to the Weber fraction for luminance. The remaining spike at the very bottom is the linear toe.
