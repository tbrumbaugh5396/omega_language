...
class Environment(dict):
    ...
    def mutate(self, key, value):
        """set! — walk the chain to update an existing binding (iterative, not recursive)."""
        env = self
        while env is not None:
            if key in env:
                if key in env.fixed:
                    raise NameError(f"Cannot mutate fixed symbol: {key}")
                env[key] = value
                return
            env = env.parent
    ...
    def is_fixed_anywhere(self, key):
        env = self
        while env is not None:
            if key in env.fixed: return True
            env = env.parent
        return False
